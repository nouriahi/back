...
public class MedecinJwtConverter implements Converter<Jwt, MedecinJwt> {

    @Override
    public MedecinJwt convert(@NonNull Jwt jwt) {
        // Extract claims and authorities as needed
        Collection<GrantedAuthority> authorities = extractAuthorities(jwt);

        // You can also map other information from the Jwt to the custom token
        var MedecinJwt = new MedecinJwt(jwt, authorities);
        MedecinJwt.setFirstname(jwt.getClaimAsString("given_name"));
        MedecinJwt.setLastname(jwt.getClaimAsString("family_name"));
        return MedecinJwt;
    }
